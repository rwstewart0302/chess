...
def ai_engine(can_castle_kingside, can_castle_queenside, board, player, valid_pieces, prev_r_delta, prev_c_end, prev_moved_piece, depth):
    ...
    white_board_state, black_board_state = board_state(player, board)

    if player == config.PLAYER_1:
        opponent = config.PLAYER_2
        max_board_state = white_board_state
        min_board_state = black_board_state
    elif player == config.PLAYER_2:
        opponent = config.PLAYER_1
        max_board_state = black_board_state
        min_board_state = white_board_state

    for d in range(depth):
        if d % 2 == 0:
            test_player = player
        elif d % 2 != 0:
            test_player = opponent

        if test_player == config.PLAYER_1:
            valid_pieces = config.PLAYER_1_PIECES
        elif test_player == config.PLAYER_2:
                ...
                for r_end in range(config.RANKS):
                    for c_end in range(config.FILES):
                        ...
                        if move_check:
                            ...
                            if test_player == player and board_state(player, temp_board) > abs(b_state):
